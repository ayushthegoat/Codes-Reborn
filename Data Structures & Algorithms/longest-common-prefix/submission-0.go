func longestCommonPrefix(strs []string) string {
    ...
    for i := range s {               // fixed: removed unused _
        for _, jval := range strs {
            if s[i] != jval[i] {
                return ans
            }
        }
        ans = ans + string(s[i])     // fixed: cannot concatenate string + byte
    }

    return ans                       // fixed: string(ans) → ans is already string
}
...
func findSmallestString(strs []string) string {
    ...
    smallest := strs[0] // start with first element
